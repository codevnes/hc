...
interface SimpleAssetsChartProps {
  data: StockAssets[];
  height?: number;
  width?: number;
  title?: string;
...
const SimpleAssetsChart: React.FC<SimpleAssetsChartProps> = ({
  data,
  height = 300,
  width = 600,
  ...
  useEffect(() => {
    ...
    // Process data to create side-by-side bars
    const processedData = sortedData.flatMap(item => {
      ...
      // Create two data points for each time point, slightly offset
      return [
        {
          time,
          value: ttsValue,
          color: '#26a69a', // Green for TTS
          type: 'tts'
        },
        {
          time,
          value: vcshValue,
          color: '#42a5f5', // Blue for VCSH
          type: 'vcsh'
        }
      ];
    });

    // Create TTS histogram series
...
export default SimpleAssetsChart;
