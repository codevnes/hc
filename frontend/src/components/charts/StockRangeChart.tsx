  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch data from the API
        const response = await axios.get(`http://localhost:5000/api/stocks/range?startDate=${formattedStartDate}&endDate=${endDate}&symbol=${symbol.toUpperCase()}`);
        ...
        if (response.data && response.data.length > 0) {
          ...
          try {
            const symbolResponse = await axios.get(`http://localhost:5000/api/stocks/symbol/${symbol.toUpperCase()}`);
