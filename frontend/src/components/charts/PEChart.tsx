...
const PEChart: React.FC<PEChartProps> = ({
  data,
  height = 300,
  width = 600,
  title = 'Biểu đồ P/E',
  ...
  useEffect(() => {
    ...
    const peData = sortedData.map(item => {
      const value = typeof item.pe === 'string' ? parseFloat(item.pe as string) : Number(item.pe || 0);
      return value;
    });
    
    const peNganhData = sortedData.map(item => {
      const value = typeof item.pe_nganh === 'string' 
        ? parseFloat(item.pe_nganh as string) 
        : Number(item.pe_nganh || 0);
      return value;
    });
