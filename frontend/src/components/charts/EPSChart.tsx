...
const EPSChart: React.FC<EPSChartProps> = ({
  data,
  height = 300,
  width = 600,
  title = 'Biểu đồ EPS',
  ...
  useEffect(() => {
    ...
    const epsData = sortedData.map(item => {
      const value = typeof item.eps === 'string' ? parseFloat(item.eps as string) : Number(item.eps || 0);
      return value;
    });
    
    const epsNganhData = sortedData.map(item => {
      const value = typeof item.eps_nganh === 'string' 
        ? parseFloat(item.eps_nganh as string) 
        : Number(item.eps_nganh || 0);
      return value;
    });
